...
class Demo(Elaboratable):
    ...
    def __init__(self):
        ...
        self.rx = Signal()
        self.start = Signal()
        self.pcm_output = Signal(16)
        self.channel_indicators = Signal(16)
    ...
    def elaborate(self, platform: Optional[Platform]) -> Module:
        ...
        for i in range(midi_notes.width):
            ...
            m.d.comb += self.channel_indicators.eq(
        ...
        m.submodules.mixer = mixer = Mixer(notes, phi,
                                           bit_depth=self.pcm_output.width)
        m.d.comb += self.pcm_output.eq(mixer.sample)
        return m
